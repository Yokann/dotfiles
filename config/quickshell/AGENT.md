...
`background`/`hoverBackground` accept either a `Colors` palette key (`"surface0"`, `"accent"`, ...) or a literal color string (`"#ff0000"`) — resolved by `Colors.resolve(token)`. `radius` and `hoverDurationMs` (the hover color transition) are plain numbers. `fontWeight` is a plain number too (Qt6's `font.weight` takes an int 1-1000 directly — `100`≈Thin, `400`=Normal, `700`=Bold, `900`=Black — no string/enum mapping needed). Unset keys fall back to `Pill`'s built-in defaults (`Colors.surface0`/`surface1`, `Metrics.radiusMedium`, `120`ms, `Font.Normal`).

`Pill` only resolves `style` — it doesn't own the `Text` elements inside it (those are each widget's own `content`, via `Pill`'s default-property alias), so **every widget built on `Pill` must itself bind its `Text`'s `font.weight: root.style.fontWeight`** (and similarly would need to for any future font-related key) for the setting to actually apply; adding a key to `Pill`'s style defaults makes it *available*, not automatically wired everywhere. All the current `Pill`-based widgets (`Clock`, `AudioIndicator`, `UpdatesIndicator`, `SysmonitorIndicator`, `PomodoroIndicator`) do this. `Button.qml` (which duplicates `Pill`'s style-resolution rather than extending it) mirrors the same `fontWeight` key independently.
...
- **Idle inhibitor**: `Quickshell.Wayland`'s `IdleInhibitor` (from the `Quickshell.Wayland._IdleInhibitor` submodule, but re-exported by the parent `Quickshell.Wayland` module so a plain `import Quickshell.Wayland` is enough) is a plain `QObject`, not a `PopupWindow`/visual item — just `enabled: bool` and `window: QObject*`. `window` takes the injected `panelWindow` contract property directly (a `PanelWindow` is an accepted `QObject*`, no coercion needed, unlike Tray's `display()` case). No service needed — it's simple enough to live entirely in the widget, same reasoning as `Workspaces.qml`. Toggling was verified to engage/disengage cleanly (no runtime errors from the Wayland object itself either way) independently of the icon-glyph problem noted in Memory above. When `inhibiting` is true, the widget overrides `Pill`'s background to `Colors.lavender` and its own `Text`'s color to `Colors.crust`, ignoring the normal hover color while active — a toggled-"on" state reads as committed, not hover-previewable. This needed one small `ui/Pill.qml` addition: `readonly property alias hovered: mouseArea.containsMouse`, since `Pill`'s hover state (`mouseArea`) is otherwise private to that file and a derived instance overriding `color` has no other way to keep its own non-active branch hover-reactive.
...
- **Tray icon vertical alignment**: `Tray.qml`'s item squares were sized as `Metrics.fontSize + Metrics.spacingMedium`, but `fontSize` (a raw `font.pixelSize`) undershoots a real rendered `Text`'s implicit height (ascent+descent+leading), which is what `ui/Pill.qml`'s `implicitHeight` is actually built from (`contentRow.implicitHeight + Metrics.spacingSmall * 2`). The mismatch (tray squares a few px shorter) made tray icons look top-aligned instead of vertically centered next to Pill-based siblings sharing the same bar-section `Row` (a plain `Row` top-aligns children of differing height — it doesn't center them, and anchors can't be added to a `Row`'s direct children to fix it there either). Fixed by measuring an actual hidden reference `Text` (same `font.family`/`pixelSize` as everywhere else) and using `heightReference.implicitHeight + Metrics.spacingSmall * 2` for the square size, matching `Pill`'s formula exactly instead of approximating it from `fontSize` alone. If a similarly-sized non-`Pill` widget is added later (`Workspaces.qml` already computes its own height independently and comes out taller, ~29px vs `Pill`'s ~24-26px, though that's a different bar section so it isn't visibly mismatched today), reach for this same "measure it, don't approximate from `fontSize`" pattern rather than guessing a constant.
